    ...
    def fetch_all(self, timeout=15):
        ...
        for i, url in enumerate(urls):
            try:
                ...
                cal = ICalendar.from_ical(resp.content)  # bytes
                for comp in cal.walk("vevent"):
                    uid = str(comp.get("uid") or f"u{i}-{total}-{int(time.time()*1000)}")
...
class SmartCalendarApp:
    ...
    def calendar_grid(self, rect):
        ...
        weeks = cal.monthdatescalendar(self.view_year, self.view_month)  # lista de semanas (4-6), cada uma com 7 datas
        # Vamos desenhar 6 linhas de calendário; se houver só 5 semanas, a última linha fica vazia
        total_rows = 6
        for r in range(total_rows):
            for c in range(7):
                x = rect.x + padding + c * day_w
                y = row_y + (r+1) * day_h  # +1 por conta da linha das labels
    ...
    def add_user_event_wizard(self):
        ...
        date_str = self.prompt("Data (YYYY-MM-DD):", default_date)
        if date_str is None:
            return
        try:
    ...
    def add_routine_wizard(self):
        ...
        rid_raw = f"routine:{int(time.time()*1000)}"
        routine = {
            "id": rid_raw,
            "title": title.strip(),
            "type": "weekly",
    ...
    def expand_routines_for_month(self, year, month):
        ...
        for r in self.routines:
            ...
            # Corrige ID não duplicar "routine:"
            rid = r.get("id") or "routine"
            if not str(rid).startswith("routine:"):
                rid = f"routine:{rid}"
